...
pub fn process(ctx: Context<PassiveLendingAccountBorrow>, amount: u64) -> Result<()> {
    msg!("Borrowing");

    let lender_bump = PassiveLendingAccount::get_lender_address(ctx.accounts.kivo_account.key()).1;

    marginfi::cpi::lending_account_borrow(
        CpiContext::new_with_signer(
            ctx.accounts.marginfi_program.to_account_info(),
            marginfi::cpi::accounts::LendingAccountBorrow {
...
#[derive(Accounts)]
pub struct PassiveLendingAccountBorrow<'info> {
